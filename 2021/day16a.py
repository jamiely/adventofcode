...
def dump_packets(packets):
    ...
    def dp(packet, depth):
        ...
        if "value" in packet:
            log.debug("%s(%d) version=%s value=%s", '.' * (depth * 2), memory["count"], packet["packet_version_decimal"], packet["decimal_value"])
        else:
            log.debug("%s(%d) version=%s subpackets=%s", '.' * (depth * 2), memory["count"], packet["packet_version_decimal"], len(packet["packets"]))
